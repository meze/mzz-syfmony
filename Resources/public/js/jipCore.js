...
(function ($){
    MZZ.jipCore = DUI.Class.create({
        ...
        open: function(url, isNew, method, params, redirect) {
            ...
            if (isNew) {
                ...
                var t = this;
                var refresh = $('<img height="16" width="16" alt="" src="' + this.imagePath + '/spacer.gif" class="refresh" />').bind('click', function() {t.refresh()});
                var status = $('<span />').append(refresh).append(' ' + url);
                this.setStatus(status);
                $(document).keydown(this.eventKey);
            } else {
                //????
            }
        ...
        close: function(windows) {
            ...
            if (this.window) {
                ...
                if(--this.windowCount == 0) {
                    this.window.kill();
                    this.window = null;
                    $(document).unbind('keydown', this.eventKey);
                    this.unlockContent();
                } else {
                    this.window.kill();
        ...
        request: function(url, type, _data) {
            var t = this;
            $.ajax({
                url: url,
                type: type,
                ...
                complete: function(data, status, request) {
                    if(status == 'success') {
                        ...
                        t.setErrorMsg(data, status, request, url, _data);
        ...
        setErrorMsg: function (transport, status, request, url, data)
        {
            if (this.window) {
                this.setTitle('ERROR');
                var reason = ((transport.status == 404) ? '404 Not found' : '') + ((transport.status == 403) ? '403 Forbidden' : '') + ((transport.status == 500) ? '500 Server sick' : '');

                reason = 'Reason: ' + (reason == '' ? 'status code ' + transport.status : reason);
                reason += '<br /><b>Contant the support and include this if you need help:</b> <br />On request: ' + url + ' with data: ' + $(data).dump();


                var message = '<h3 style="text-align: center; color: #bd2025">' + MZZ.jipI18n[JIP_LANG].error + '</h3><p align="center">' + reason + '</p>';
        ...
        setLoadingMsg: function()
        {
            if (this.window) {
                ...
                this.window.show().content('<div id="jipLoad"><img src="' + this.imagePath + '/jip/loader.gif" width="220" height="19" /><br />' + MZZ.jipI18n[JIP_LANG].loading + '<br /><a href="javascript: void(jipWindow.close());">' + MZZ.jipI18n[JIP_LANG].cancel + '</a></div>');
            }
        },
        ...
        setRefreshMsg: function()
        {
            if (this.window) {
                ...
                this.window.show().content('<div id="jipLoad"><img src="' + this.imagePath  + '/jip/loader.gif" width="220" height="19" /><br />' + MZZ.jipI18n[JIP_LANG].refresh + '</div>');
            }
        },
    ...
    //$('a.jipLink').live('click', MZZ.jipLinkObserver); //old links, DEPRECATED
    ...
    MZZ.jipI18n = {
        en: {
            ...
            noWindow: 'No window to open the page in.'
