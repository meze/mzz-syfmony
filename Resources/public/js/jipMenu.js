    ...
    MZZ.jipMenu = DUI.Class.create({
        ...
        showLang: function(parent, lnk, langs) {
            ...
            if (this.cLangId == id) {
                ...
                if ((size_p.width + pos_p.left + size_l.width) > (size_d.width + pos_d.left)) {
                    x = pos_p.left - size_l.width;
                } else {
                    x = pos_p.left + size_p.width;
                }
            ...
            if (!$('#' + id).length) {
                ...
                for (var i = 0, l = items.length; i < l; i++) {
                    ...
                    if (elm[2] == '') {
                        ...
                        $('<img src="' + elm[2] + '" alt="." width="16" height="16" />').appendTo(jipMenuItemA);
                    }
        ...
        itemClick: function (e) {
            ...
            if (e.data.target) {
                ...
                if (e.data.handler)
                    return jipMenu._executeFunctionByName(e.data.handler, window, e.data.link);
                else
                    return jipWindow.open(e.data.link);
            }

            //e.data.target.handler;
            return false;
        },
